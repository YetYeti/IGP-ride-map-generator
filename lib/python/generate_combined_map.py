...
def generate_single_track(gps_data: List[Tuple[float, float]], output_path: str, track_width: int) -> bool:
    """生成单个轨迹图"""
    if not gps_data:
        return False
...
def generate_combined_map(image_files: List[str], output_path: str, margin: int, columns: int) -> bool:
    """生成合并轨迹大图"""
    if not image_files:
        return False
...
def main():
    ...
    try:
        ...
        for i, fit_file in enumerate(args.fit_files, 1):
            print_progress(f"处理 {i}/{len(args.fit_files)}: {os.path.basename(fit_file)}")
            
            gps_data = extract_gps_data(fit_file)
            
            if not gps_data:
                print_progress(f"跳过无GPS数据的文件: {os.path.basename(fit_file)}")
                continue
        ...
        if generated_images:
            ...
            success = generate_combined_map(generated_images, args.output_path, args.margin, args.columns)
